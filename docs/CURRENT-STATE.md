...
核对日期：2026-09-13。**2.1.0** 的多栏工作台与发行配置已通过 [PR 49](https://github.com/KRPCT/InkStream/pull/49) 合并至 main（合并提交 `491a235`）。[发布提交三平台 CI](https://github.com/KRPCT/InkStream/actions/runs/34737464772) 已通过。按用户要求，合并后重构 README，并将它纳入 2.1.0。安装包与签名更新清单的就绪状态见 [2.1.0 发行页](https://github.com/KRPCT/InkStream/releases/tag/v2.1.0)。
...
此前等待合并的 24 个实现票已随 2.0.0 关闭。以下 11 个专项票仍开放，发布或 CI 通过不取代其验收。
...
已实现项目轨、概览／文稿／文献／版本目的地、文献详情与明确插入、键盘标签导航、布局和显示收敛。1910 项前端断言、41 项显式绑定、类型/Lint/前端/原生构建通过；三轮 ComputerUse 已完成并清理进程。实际截图、发现与修复、证据限制见 [验收记录](WORKBENCH-ACCEPTANCE.md)。这些工作台行为验收对应开发提交 `09b32ff`，其[三平台 CI](https://github.com/KRPCT/InkStream/actions/runs/34719252134)已通过；2.1.0 追加版本与发行说明。合并已经完成，后续 README 调整仅涉及文档；合并与发行入口见本页顶部。
