...
describe('统一冻结门：组合期零 setState/reload/write，end 后恰好一次', () => {
  ...
  it('drain 三段全序：refreshLivePreview → reload → write（§2.4 固定顺序含 write 段）', async () => {
    vi.useFakeTimers();
    const seq: string[] = [];
    reloadFromDisk.mockImplementation(() => {
      seq.push('reload');
    ...
    try {
      dispatchComposition(view, { phase: 'compositionstart', data: '你' });
      // 组合期先来外部变更（排 reload），后 autosave 定时器到期（排 write）：入队序 reload→write。
      await arbitrateVaultChange({ path: ABS, kind: 'modify' });
      scheduleAutosave(REL);
      await vi.advanceTimersByTimeAsync(600);
      expect(seq).toEqual([]); // 组合期三段全部按住

      dispatchComposition(view, { phase: 'compositionend', data: '你好' });
      await vi.runAllTimersAsync();
      await Promise.resolve();
      await Promise.resolve();
      expect(seq).toEqual(['refresh', 'reload', 'write']);
    } finally {
      spy.mockRestore();
      vi.useRealTimers();
    }
