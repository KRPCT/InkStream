...
// 原生 IME 武装命令（arm_webview_ime）在 jsdom 下无 Tauri 桥：mock invoke 以断言「焦点路径
// 调用了它一次」。jsdom 无法验证真实 TSF 武装——真机验收由编排层 CDP 检查（首次拼音
// compositionstart）完成，此处仅校验前端调用形状（focus + 选区重申 + invoke 各一次）。
vi.mock('../ipc/invoke', () => ({
...
describe('openFile 聚焦编辑器 contentEditable（IME 吞字修复，EDIT-06）', () => {
  ...
  it('openFile 在无模态时经双重 rAF 聚焦并武装原生 IME', () => {
    const view = mountView();
    const focusSpy = vi.spyOn(view, 'focus');
    openFile(view, 'focus.md', 'hello', baseExtensions());
    expect(focusSpy).toHaveBeenCalledTimes(1);
    // 焦点路径 fire-and-forget 调原生 arm_webview_ime 一次（无参）。
    expect(invoke).toHaveBeenCalledTimes(1);
    expect(invoke).toHaveBeenCalledWith('arm_webview_ime', undefined);
    view.destroy();
  });

  it('命令面板打开时 openFile 不抢焦点（不聚焦、不武装 IME）', () => {
    const view = mountView();
    const focusSpy = vi.spyOn(view, 'focus');
    usePaletteStore.getState().openPalette();
    openFile(view, 'focus2.md', 'hello', baseExtensions());
    expect(focusSpy).not.toHaveBeenCalled();
    // 模态守卫跳过整条聚焦路径：原生武装也不发。
  ...
  it('imeSafeFocus 不再派发合成 pointer/mouse 往返（已证无效，移除）', () => {
    const view = mountView();
    const seen: string[] = [];
    ...
    // 合成指针往返实测不武装 WebView2/TSF，已删除：聚焦路径只剩 view.focus() + 原生武装。
    expect(seen).toEqual([]);
    expect(focusSpy).toHaveBeenCalledTimes(1);
    expect(invoke).toHaveBeenCalledWith('arm_webview_ime', undefined);
    view.destroy();
