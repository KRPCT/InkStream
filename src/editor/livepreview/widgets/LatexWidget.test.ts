...
describe('LatexWidget', () => {
  it('eq 按 latex + 就绪态比较', () => {
    expect(new LatexWidget('\\frac{a}{b}').eq(new LatexWidget('\\frac{a}{b}'))).toBe(true);
    expect(new LatexWidget('\\frac{a}{b}').eq(new LatexWidget('\\frac{c}{d}'))).toBe(false);
  });

  it('空块 → 占位（不调 convert）', () => {
    ready = true;
    const dom = new LatexWidget('   ').toDOM(fakeView);
  ...
  it('就绪 → MathJax convert 节点挂进 mount', () => {
    ready = true;
    const dom = new LatexWidget('\\frac{a}{b}').toDOM(fakeView);
  ...
  it('未就绪 → 加载中占位显源码 + 触发懒加载', () => {
    ready = false;
    const dom = new LatexWidget('x^2').toDOM(fakeView);
