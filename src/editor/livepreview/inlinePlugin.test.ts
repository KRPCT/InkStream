...
describe('inlinePlugin 列表 / 引用逐行还原（D-06）', () => {
  it('无序列表：`-`/`*`/`+` 标记 replace 为项目符号 widget + 行级 cm-ink-list-line', () => {
    view = lpView('- a\n* b\n+ c');
    // 光标默认在 0（第一项行）；移到末行让前两项渲染。
    cursorToEnd(view);
    const decos = collectDecos(view);
    // 第一行 `-`（0-1）、第二行 `*`（4-5）replace 为项目符号 widget（非活动行）。
    expect(decos.some((d) => d.widget && d.from === 0)).toBe(true);
    expect(decos.some((d) => d.widget && d.from === 4)).toBe(true);
    // 每个列表项行得 cm-ink-list-line 行级装饰（零宽 line deco，悬挂缩进保排版）。
    const listLines = decos.filter((d) => d.class === 'cm-ink-list-line' && d.from === d.to);
    expect(listLines.length).toBe(3);
  });

  it('有序列表：序号 `1.`/`2.` 保留为可见文本（不 replace、不隐藏）+ 行级 cm-ink-list-line', () => {
    view = lpView('1. a\n2. b\n3. c');
    cursorToEnd(view); // 光标末行，前两项渲染。
    const decos = collectDecos(view);
    // 第一行序号 `1.`（0-2）不被 widget/hidden 覆盖：序号保留可见。
    expect(decos.some((d) => d.from === 0 && (d.widget || d.class === 'cm-ink-hidden'))).toBe(false);
    expect(decos.some((d) => d.from === 5 && (d.widget || d.class === 'cm-ink-hidden'))).toBe(false);
    // 但仍得列表行级装饰（缩进排版一致）。
    expect(decos.filter((d) => d.class === 'cm-ink-list-line').length).toBe(3);
  });
  ...
  it('列表活动行：保留 cm-ink-list-line 行级装饰（缩进稳定）但标记本体零 mark/replace（源码可见可编辑），其余项保渲染', () => {
    view = lpView('- a\n- b');
    // 光标在第一行（pos 0）：第一项活动 → 保行级 line 装饰、`-` 不替换；第二项（4-5）保渲染。
    view.dispatch({ selection: EditorSelection.cursor(0) });
    const decos = collectDecos(view);
    // 第二行 `-`（4-5）仍 replace 为项目符号 widget。
    expect(decos.some((d) => d.widget && d.from === 4)).toBe(true);
    // 第一行（活动）：保留零宽 cm-ink-list-line 行级装饰（缩进与非活动态一致，杜绝抖动）。
    const line1 = view.state.doc.line(1);
    const onLine1 = decos.filter((d) => d.from >= line1.from && d.from <= line1.to);
    expect(onLine1.some((d) => d.class === 'cm-ink-list-line' && d.from === d.to)).toBe(true);
    // 但标记本体零 mark/replace/widget：`-` 源码可见可编辑（findCompositionRange 文本相等闸门）。
    expect(onLine1.filter((d) => d.from !== d.to).length).toBe(0);
