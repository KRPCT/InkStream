...
export function useIndexQuery<T>(load: () => Promise<T>, enabled = true, reload = 0) {
  ...
  const [result, setResult] = useState<{ key: string; load: (() => Promise<T>) | null; data: T | null; error: string | null }>({
