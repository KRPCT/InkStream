...
/**
 * 编辑器右键上下文菜单（R4 §4.3）：复用通用 Menu 组件 + registry 数据驱动（与 MenuBar 同框架）。
 *
 * 固定定位到 contextmenu 坐标；段落/格式/插入子菜单仅 markdown 家族文档可用（非 markdown 文档
 * disabled）。剪贴板/查找走 edit.* 命令。组合期防御在 EditorArea 的 contextmenu handler——
 * 组合中（isComposing）根本不开此菜单（铁律：组合期不 dispatch 破坏性操作）。
 */
export default function EditorContextMenu({ position, onClose }: Props) {
  const [, setVersion] = useState(0);
  // 注册表变更时刷新（菜单开启期间命令增删的边界情形，与 MenuBar 同纪律）。
  useEffect(() => subscribe(() => setVersion((v) => v + 1)), []);

  const commands = new Map(getAll().map((c) => [c.id, c]));
  const items = buildEditorMenu(commands, isMarkdownFamily(), onClose);
