...
export const useGitStore = create<GitState>((set, get) => ({
  ...
  setRepoRoot: (root) => {
    ...
    set({ repoRoot: root, status: null, branches: [], loading: false });
    void get().refresh();
  },
  refresh: async () => {
    ...
    try {
      ...
      if (get().repoRoot !== root || owner !== generation || request !== refreshSequence) return;
      set({ status, branches, loading: false });
    } catch {
      // 非 git / 读失败：保持空，不抛 UI（与 GitGuidanceBar 的「非 git→引导 init」协同）。
      if (get().repoRoot === root && owner === generation && request === refreshSequence) set({ loading: false });
    }
  },
  scheduleRefresh: () => {
